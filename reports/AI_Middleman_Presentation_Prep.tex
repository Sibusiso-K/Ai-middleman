...
\begin{itemize}
    ...
    \item Multilingual support was traced to its real root cause --- Stage 1's keyword filter, not the more obvious draft/image-prompt issues --- and fixed with zero added latency for the common English case (Section~\ref{sec:multilingual}).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Multilingual support is now real but unevenly verified.} It works architecturally for all 11 languages, but only two have actually been tested end-to-end --- Afrikaans came back fluent, isiZulu had real grammatical rough edges. No eval-harness coverage and no native-speaker verification exist yet for any of the 11.
