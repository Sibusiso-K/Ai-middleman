...
6 & Image description hardcodes ``in plain English,'' blocking multilingual image support & Functional gap & \warnc{Documented, not fixed} \\
7 & ngrok tunnel has no auto-restart --- died silently mid-session, dashboard falsely showed ``offline'' even though the API and DB were healthy the whole time (both badges are driven by one \texttt{/health} check, not independent DB verification) & Operational & Restarted; no supervisor added \\
8 & No automated evaluation harness existed for intent/matching/draft quality before this week & \badc{The single biggest gap for a data-science-role narrative} & \good{Now exists} --- see Section~\ref{sec:eval} \\
...
\begin{center}
{\Large \textbf{7/10} --- a strong, thoughtfully-designed MVP, not yet a production system.}
\end{center}

\subsection{What earns the 7}
\begin{itemize}
    \item The two-stage retrieve-then-rank pipeline is textbook-correct architecture.
    \item The human-in-the-loop approval gate (Send/Edit/Skip) is a genuinely mature safety decision most student projects skip entirely.
    \item The thread/event data model is the right abstraction for multi-turn conversation state.
    \item A real small-model failure mode (the merged rank+draft prompt) was diagnosed and fixed with the right engineering response --- split it.
    \item The fallback-provider chain shows resilience thinking, even though it took a live stress test to reveal it wasn't actually validated end-to-end.
\end{itemize}

\subsection{What caps it at 7, not 9--10}
\begin{itemize}
    \item \textbf{No evaluation harness existed until this week.} Even now it's a small, self-authored labeled set --- not independently blind, not adversarial, not benchmarked against production-scale message diversity.
    \item \textbf{The resilience bug found during testing} means the fallback path --- the thing meant to prevent an outage --- had never actually been exercised end-to-end before being tested this week. ``We built the safety net but never tested it holds weight'' is worth naming honestly if asked how reliability is validated.
    \item \textbf{No multilingual support yet}, despite this being a South African contact network --- a real functional gap for the actual target market.
    \item \textbf{Everything rides on two free-tier APIs} with no paid or self-hosted fallback --- fine for a vac project, a real constraint to name plainly if asked how this would scale.
\end{itemize}

None of these are embarrassing --- they are exactly the kind of gaps a strong junior candidate is expected to name unprompted. Doing that, instead of pretending the project is finished, will land far better with the room than overselling it.

\section{Multilingual roadmap (South Africa's 11 official languages) --- not yet implemented}

\textbf{Current state:}
\begin{itemize}
    \item \textbf{Voice notes:} Whisper already auto-detects language and transcribes in that language --- no code change needed for basic function. Expect strong results for English/Afrikaans, workable results for isiZulu/isiXhosa/Sesotho, and materially weaker accuracy for isiNdebele, Xitsonga, Tshivenda, siSwati, Sepedi, Setswana --- these are lower-resource languages for \emph{every} open transcription model, not a bug in this codebase.
    \item \textbf{Image transcription:} hardcodes ``describe this image \dots\ in plain English'' --- an easy, safe fix (remove that phrase) not yet applied.
    \item \textbf{Text replies:} nothing currently detects the incoming message's language or asks the model to reply in kind --- every prompt in the pipeline is English-only today.
\end{itemize}

\textbf{The plan, if asked ``what's next'':}
\begin{enumerate}
    \item Detect the language of the incoming message and instruct the draft model to reply in kind, code-switching naturally with English company/role names (mirrors how South Africans actually speak).
    \item Drop the hardcoded ``in plain English'' in the image description prompt.
    \item Test each language once with a native or near-native speaker before claiming full support --- an 8B model's fluency in Xitsonga or Tshivenda is genuinely uncertain, and claiming ``full support for all 11 languages'' without verifying even one example in the weaker languages is a real credibility risk with this specific audience.
\end{enumerate}
...
``What's the biggest risk in this design?'' & Everything depends on free-tier LLM APIs with rate limits; the fallback chain exists but had a real bug (fixed this week) where it silently used the wrong timeout for the backup provider. \\
``What would you build next?'' & An evaluation harness with a larger, ideally human-labeled test set, and multilingual support for South Africa's 11 official languages --- both are scoped and neither is implemented yet. \\
