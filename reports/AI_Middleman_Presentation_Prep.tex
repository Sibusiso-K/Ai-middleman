...
\subsection{Flow 5 --- multilingual (South Africa's 11 languages)}
\begin{itemize}
    \item Afrikaans: ``Ken jy iemand wat 'n korporatiewe prokureur is in Londen?''
    \item isiZulu: ``Ngidinga ummeli waseLondon wamashishini''
\end{itemize}
The reply comes back in the same language. \textbf{Be honest:} Afrikaans is fluent; isiZulu is understandable but grammatically rough --- an 8B-model limitation, not a bug.
...
8 & Vision + Close & 1m & Multilingual for SA's 11 languages; callback to Alex; thank you. \\
\bottomrule
\end{longtable}
...
11 & ``Next: 11 languages, real auth, scale'' & SA flag / language names & Beat 8, vision. \\
...
\begin{description}[leftmargin=0pt, style=nextline]
  ...
  \item[``An 8B model's isiZulu was rough --- so is the multilingual claim real?''] \textbf{Strong:} ``The \emph{architecture} supports all 11 equally --- detect, translate-for-search, reply-in-kind. \emph{Quality} isn't uniform: Afrikaans is fluent, isiZulu is understandable but grammatically rough, and that tracks training data, an industry-wide limit. I'd verify each language with a native speaker before claiming more.''
