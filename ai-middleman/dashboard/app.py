# dashboard/app.py
# Streamlit dashboard for AI Middleman — contact database visualisation & match testing
...
st.set_page_config(
    page_title="AI Middleman Dashboard",
    page_icon="🤝",
    layout="wide",
...
page = st.sidebar.radio(
    "Navigation",
    ["📊 Database Overview", "🔍 Match Tester", "📋 Contact Browser", "📈 Analytics"],
)
...
try:
    ...
    if resp.status_code == 200:
        ...
        st.sidebar.warning(f"🟡 API returned {resp.status_code}")
except Exception:
...
if page == "📊 Database Overview":
    ...
    st.markdown("Key statistics and distributions across the contact database.")
    ...
    col3.metric("Avg Relationship Strength", str(avg_rel))
    ...
    with col_right:
        st.subheader("Contacts by Location (Top 10)")
        loc_data = run_query("""
        ...
        if loc_data:
            ...
            fig = px.bar(df_loc, x="location", y="cnt", color="cnt",
    ...
    with col_right2:
        st.subheader("Preferred Contact Channel")
        chan_data = run_query("""
            SELECT preferred_contact_channel, COUNT(*) AS cnt
            FROM contacts
            WHERE preferred_contact_channel IS NOT NULL AND preferred_contact_channel != ''
            GROUP BY preferred_contact_channel
            ORDER BY cnt DESC
        """)
        if chan_data:
            df_chan = pd.DataFrame(chan_data)
            fig = px.bar(df_chan, x="preferred_contact_channel", y="cnt", color="cnt",
...
# ═══════════════════════════════════════════════════════════════════
    ...
    st.title("🔍 Match Tester")
    st.markdown("Test the matching engine with natural language queries.")

    query = st.text_area(
        "Describe who you are looking for...",
        placeholder="e.g. I need a credit finance specialist in London",
        height=100,
    )
    ...
    if st.button("🔎 Find Matches", type="primary", use_container_width=True):
        if not query.strip():
            ...
            with st.spinner("Searching contacts and ranking with AI..."):
                try:
                    resp = requests.post(
                        f"{API_URL}/match",
                    ...
                    if resp.status_code == 200:
                        ...
                        st.info(f"🔎 Keyword filter found **{candidates_count}** candidates before LLM ranking")

                        if match_quality == "none":
                            st.warning(f"⚠️ {clarification or 'No matches found. Try a different query.'}")
                        else:
                            st.success(f"Found {len(matches)} match(es) — Quality: **{match_quality.upper()}**")
                            ...
                            for m in matches:
                                ...
                                with st.container():
                                    ...
                                    with cols[0]:
                                        ...
                                        st.markdown(f"### {m.get('name', 'Unknown')}{vip_badge}")
                                    ...
                                    with cols[1]:
                                        ...
                                        st.progress(conf, text=f"{conf_pct}%")

                                    st.caption(f"💬 {m.get('reasoning', '')}")
...
# ═══════════════════════════════════════════════════════════════════
    ...
    st.markdown("Browse and search the full contact database.")
    ...
    st.caption(f"Showing {len(results)} contact(s)")
    ...
    if results:
        ...
        st.dataframe(df_display.iloc[start:end], width='stretch', hide_index=True)
        ...
        if selected_name:
            ...
            with col_b:
                ...
                st.markdown(f"**Channel:** {contact['preferred_contact_channel']}")
...
# ═══════════════════════════════════════════════════════════════════
    ...
    st.title("📈 Analytics")
    st.markdown("Deeper insights into the contact network.")
    ...
    # ── Line: contacts by last_contacted month ──
    st.subheader("Contacts by Last Contacted Month")
    activity_data = run_query("""
        SELECT TO_CHAR(last_contacted, 'YYYY-MM') AS month, COUNT(*) AS cnt
        FROM contacts
    ...
    if activity_data:
        ...
        fig = px.line(df_act, x="month", y="cnt", markers=True)
        fig.update_layout(height=400, xaxis_title="Month", yaxis_title="Contacts Contacted")
        st.plotly_chart(fig, width='stretch')
    else:
    ...
    intros_data = run_query("""
        SELECT full_name, title, company, sector, intros_made
        FROM contacts
    ...
    if intros_data:
        ...
        df_intros.columns = ["Name", "Title", "Company", "Sector", "Intros Made"]
        st.dataframe(df_intros, width='stretch', hide_index=True)
    else:
